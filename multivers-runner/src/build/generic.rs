...
impl Executable for Build<'_> {
    unsafe fn exec(
        ...
        // Grab the path of the FAT wrapper before we spawn the inner payload
        let current_wrapper_exe = std::env::current_exe().unwrap_or_default();

        let exit_status = Command::new(&exe_path)
            .env("MULTIVERS_RUNNER_PATH", current_wrapper_exe)
            .args(args)
            .status()
            .map_err(|_| {
                proc_exit::Code::FAILURE.with_message(format!(
